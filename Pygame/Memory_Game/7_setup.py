...
def display_start_screen():
    ...
    message = game_font.render(f"{current_level}", True, WHITE)
    message_rect = message.get_rect(center = start_button.center)
...
def check_number_buttons(pos):
    ...
    # 모든 숫자를 다 맞혔다면? 레벨을 올리고 시작화면으로 이동
...
def game_over():
    ...
    message = game_font.render(f"Your level is {current_level}", True, WHITE)
